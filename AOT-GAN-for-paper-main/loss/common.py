...
class VGG19(nn.Module):
    def __init__(self, resize_input=False, device='cuda'):
    ...
    def forward(self, x):
        ...
        # ===== НОВОЕ: явно отключить градиенты внутри forward =====
        with torch.no_grad():
            features = []
...
class GaussianBlur(nn.Module):
    ...
    def __init__(self, kernel_size, sigma, device='cuda'):
        ...
        # ===== КРИТИЧНО: создать kernel один раз =====
    ...
    def forward(self, x):
        ...
        # ===== ОПТИМИЗИРОВАНО: kernel уже на device =====
        b, c, h, w = x.shape
        tmp_kernel = self.kernel.to(x.dtype)  # только приводим тип, не копируем
...
def gaussian_blur(input, kernel_size, sigma, device='cuda'):
    r"""Function that blurs a tensor using a Gaussian filter.
    See :class:`~kornia.filters.GaussianBlur` for details.
    """
    return GaussianBlur(kernel_size, sigma, device=device)(input)
